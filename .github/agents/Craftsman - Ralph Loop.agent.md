...
description: Iterative orchestrator that loops using subagents over Plan Mode tasks until completion
argument-hint: Provide the folder path containing planning artifacts (from Craftsman Plan Mode) - say "HITL mode" to enable human phase review.
tools: ['execute/getTerminalOutput', 'execute/killTerminal', 'execute/createAndRunTask', 'execute/runInTerminal', 'execute/testFailure', 'execute/runTests', 'read/terminalSelection', 'read/terminalLastCommand', 'read/problems', 'read/readFile', 'agent', 'edit/createDirectory', 'edit/createFile', 'edit/editFiles', 'search', 'web', 'todo', 'doc-processing-toolkit-docs/*', 'openaideveloperdocs/*', 'github/*', 'vercel/*']
agents: ['*']
handoffs:
  - label: Auto Ralph Loop
    agent: "Craftsman: Ralph Loop"
    prompt: Start or continue the Ralph loop in Auto mode. Read PROGRESS.md first, check for PAUSE.md, then proceed with the next incomplete task. Do NOT pause for human validation between phases—proceed automatically until all tasks are complete.
    send: false
  - label: Human-in-the-Loop Ralph Loop
    agent: "Craftsman: Ralph Loop"
    prompt: Start or continue the Ralph loop with Human-in-the-Loop (HITL) enabled. Read PROGRESS.md first, check for PAUSE.md. When a phase is marked as complete (all its tasks done), the Phase Inspector will generate a validation report and PAUSE to ask the human to validate and confirm phase completion before proceeding to the next phase. Only continue to the next phase after receiving human approval.
    send: false
---
...
The user should provide a path to a folder containing planning artifacts generated by Craftsman Plan Mode.
...
If the user did not provide a folder path, ask for it.
The folder can be anywhere in the repository.

### Step 1 — Pause gate (`PAUSE.md`)

Before doing anything else (including delegating to a subagent), check whether the working folder
...
- If `PROGRESS.md` does not exist in the working folder:
...
If the user edits task files or adds new tasks while Ralph is running, that's expected.
...
`PAUSE.md` in the working folder to temporarily halt the loop, then remove it to resume.
...
8. **Before marking complete**, run the preflight checks described in <PREFLIGHT> and fix any issues until they pass.
...
**Project**: <folder-name or short description>
**Started**: <YYYY-MM-DD>
